...
class NiiToTensor:
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            ...
            if len(img.shape) < 4:
                img = np.expand_dims(img, -1)
            results[key] = to_tensor(img).permute(3, 0, 1, 2).contiguous()
        return results
    ...
    def __call__(self, results):
        ...
        if 'img' in results:
            ...
            if not img.flags.c_contiguous:
                ...
                img = to_tensor(img).permute(3, 0, 1, 2).contiguous()
            results['img'] = DC(
                img, padding_value=self.pad_val['img'], stack=True, pad_dims=3)
        for key in ['proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels']:
            if key not in results:
                continue
            results[key] = DC(to_tensor(results[key]), pad_dims=3)
        if 'gt_masks' in results:
            results['gt_masks'] = DC(
                results['gt_masks'],
                padding_value=self.pad_val['masks'],
    ...
    def __call__(self, results):
        ...
        for key in self.meta_keys:
            if key in results:
                ...
                # print(f'there is no key {key} in results')
                continue

        data['img_metas'] = DC(img_meta, cpu_only=True, pad_dims=3)
        for key in self.keys:
            data[key] = results[key]
        return data
