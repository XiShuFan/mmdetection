...
class LoadNiiFromFile:
    ...
    def __call__(self, results):
        ...
        # 读取出itk格式数据
        img_itk = sitk.ReadImage(fileName=filename)
        # 转换成numpy格式
...
class LoadNiiAnnotations:
    ...
    def _load_bboxes(self, results):
        ...
        if self.denorm_bbox:
            ...
            if bbox_num != 0:
                d, h, w = results['img_shape'][:3]
                results['gt_bboxes'][:, 0::3] *= w
                results['gt_bboxes'][:, 1::3] *= h
                results['gt_bboxes'][:, 2::3] *= d
    ...
    def _poly2mask(self, mask_ann, img_d, img_h, img_w):
        """Private function to convert masks represented with polygon to
        bitmaps.

        Args:
            mask_ann (list | dict): Polygon mask annotation input.
            img_d (int): The depth of output mask.
            img_h (int): The height of output mask.
            img_w (int): The width of output mask.

        Returns:
            numpy.ndarray: The decode bitmap mask of shape (img_d, img_h, img_w).
        """
    ...
    def _load_masks(self, results):
        ...
        d, h, w = results['img_info']['depth'], results['img_info']['height'], results['img_info']['width']
