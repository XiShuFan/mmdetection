...
class BboxOverlaps3D:
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
        ...
        Args:
            ...
            bboxes2 (Tensor): bboxes have shape (n, 6) in <x1, y1, z1, x2, y2, z2>
                format, shape (n, 7) in <x1, y1, z1, x2, y2, z2, score> format, or be
                empty.
...
def bbox_overlaps_3d(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    Note:
        ...
        using bbox_overlaps_3d function:
    ...
    Example:
        ...
        >>>     [10, 10, 20, 20],
        >>> ])
        >>> overlaps = bbox_overlaps_3d(bboxes1, bboxes2)
        >>> assert overlaps.shape == (3, 3)
        >>> overlaps = bbox_overlaps_3d(bboxes1, bboxes2, is_aligned=True)
        >>> assert overlaps.shape == (3, )

    Example:
        >>> empty = torch.empty(0, 4)
        >>> nonempty = torch.FloatTensor([[0, 0, 10, 9]])
        >>> assert tuple(bbox_overlaps_3d(empty, nonempty).shape) == (0, 1)
        >>> assert tuple(bbox_overlaps_3d(nonempty, empty).shape) == (1, 0)
        >>> assert tuple(bbox_overlaps_3d(empty, empty).shape) == (0, 0)
    """
    ...
    ious = overlap / union
    # ious_array = ious.cpu().numpy()
    # 输出不同iou_thr下正样本的数量
    # for i in range(1, 10):
    #     print(f'[iou3d_calculator] iou_thr={0.1*i} positive_sample={(ious_array > 0.1*i).sum()}')
    if mode in ['iou', 'iof']:
        return ious
    # calculate gious
    enclose_whd = fp16_clamp(enclosed_rb - enclosed_lt, min=0)
